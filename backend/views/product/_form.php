    ...
    <div class="row">
        <div class="col-lg-8">
            <div class="input-group-control">
                ...
                <?= $form->field($model, 'price_discount')->textInput(['type' => 'number','maxlength' => true]) ?>
                <?= $form->field($model, 'quantity_in_stock')->textInput(['type' => 'number','maxlength' => true]) ?>
                <?= $form->field($model, 'origin')->textInput(['maxlength' => true]) ?>
                <?= $form->field($model, 'weight')->textInput(['type' => 'number','maxlength' => true]) ?>
                <?= $form->field($model, 'width')->textInput(['type' => 'number','maxlength' => true]) ?>
                <?= $form->field($model, 'height')->textInput(['type' => 'number','maxlength' => true]) ?>
                <?= $form->field($model, 'length')->textInput(['type' => 'number','maxlength' => true]) ?>
                <?= $form->field($model, 'fee_ship')->textInput(['type' => 'number','maxlength' => true]) ?>
                <?= $form->field($model, 'is_highlight')->checkbox(['uncheck' => 0, 'value' => 1]); ?>
                <?= $form->field($model, 'status')->checkbox(['uncheck' => 0, 'value' => 1, 'label' => 'Hiển thị',]); ?>
            </div>
        </div>
        <div class="col-lg-4">
            <?= $form->field($model, 'category_id')->dropDownList(Category::getListCategory(),['prompt' => Yii::t('app', 'Chọn chuyên mục'), 'class' => 'form-control select2 select2-hidden']) ?>
            <?= $form->field($model, 'origin')->dropDownList(Yii::$app->params['product_origin'],['prompt' => Yii::t('app', 'Chọn tình trạng'), 'class' => 'form-control select2 select2-hidden']) ?>
            <div id="gallery">
